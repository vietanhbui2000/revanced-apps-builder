...
merge_splits() {
	...
	if ! OP=$(java -jar "$APKSIGNER" sign --ks ks_pkcs12.keystore --ks-pass pass:1234567890 --key-pass pass:1234567890 --ks-key-alias vietanhbui2000 \
...
--keystore-entry-password=1234567890 --keystore-password=1234567890 --signer=vietanhbui2000 --keystore-entry-alias=vietanhbui2000 -t '$tmp_files' $patcher_args"
...
build_rv() {
	...
	for build_mode in "${build_mode_arr[@]}"; do
		...
		if [ -n "$microg_patch" ]; then
			patched_apk="${TEMP_DIR}/${app_name_l}_${version_f}-${arch_f}_${build_mode}_${rv_brand_f}.apk"
		else
			patched_apk="${TEMP_DIR}/${app_name_l}_${version_f}-${arch_f}_${rv_brand_f}.apk"
		fi
		if [ -n "$microg_patch" ]; then
		...
		local apk_output="${BUILD_DIR}/$(basename "$patched_apk")"
